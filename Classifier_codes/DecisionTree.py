...
class DecisionTreeClassifier:
    ...
    def __init__(self, min_samples_split=2, max_depth=100, n_feats=None):
        ...
        self.n_feats = n_feats # Number of features to consider at each split (for Random Forest)
        self.root = None
        self._n_features = None # <-- FIX 1: Add a variable to store feature count
    ...
    def fit(self, X, y):
        ...
        # Determine number of features to use at each split:
        n_total_features = X.shape[1]
        self._n_features = n_total_features # <-- FIX 2: Store the number of features
        self.n_feats = n_total_features if self.n_feats is None or self.n_feats >= n_total_features else self.n_feats
    ...
    def predict(self, X):
        ...
        # <-- FIX 3: Check against the stored feature count -->
        if self._n_features is None:
             raise RuntimeError("Model fitted but feature count not stored. This shouldn't happen.")
        
        if X.shape[1] != self._n_features:
             raise ValueError(f"Input feature dimension mismatch: Expected {self._n_features}, got {X.shape[1]}")
        # <-- END OF FIX -->
        ...
        if np.any(predictions == None):
             print("Warning: Some predictions resulted in None. Replacing with default (0).")
             predictions[predictions == None] = 0 
    ...
    # <-- FIX 4: Removed the broken _get_num_features method -->